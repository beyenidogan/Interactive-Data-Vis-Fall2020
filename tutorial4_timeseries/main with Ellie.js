...
const width = window.innerWidth * 0.7,
  ...
  radius = 3,
  default_selection = "All Boroughs";
...
d3.csv("../data/Film_Permits.csv", d => {
  //const date = d3.utcParse("%m/%y/%d %H:%M")(d.StartDateTime)
  ...
  return ({
    ...
    //month: d3.timeFormat("%M")(date),
    //month: date.getMonth()+1,
    //day = d3.timeFormat("%a %d")(d.StartDateTime),
    //week = d3.timeFormat("%b %d")(d.StartDateTime),
    //month = d3.utcParse("%M")(d.StartDateTime),
    //year = d3.timeParse("%Y")(d.StartDateTime),
    Borough: d.Borough,
    ZipCodes: d.ZipCodes,
    parsedZips: zipcodes,
    Category:d.Category,
  ...
  state.data = raw_data;
  //state.data.map(line => line.ZipCodes?.split(",")?.map(zipcode=> ZipCodes.add(zipcode)));
  state.data.filter(d => d.EventTypes == "Shooting Permit");
  console.log("filtered", raw_data);
  init();
});
...
function init() {
  ...
  /* state.yearAggregate = state.data.reduce((t,v) => {
    if (Object.keys(t).includes(v.year)) {
      return {
        [v.year]: [...t[v.year], v]
      }
    } else {
  ...
  //console.log(state.yearAggregate)
